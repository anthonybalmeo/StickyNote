...
class ActiveRecord
{
    ...
    public function __construct($table, $key = null, $col = 'id')
    {
        ...
        //If a key is set, then it should belong to the row data. This is 
        //necessary for a successful create operation in cases where the index
        //is not an autoincrementing value.
        if($key != null)
        {
            $this->$col = $key;
        }
    ...
    /**
     * Returns the key of the current active record.
     * @return mixed The key of the current active record.
     */
     ...
     *                         not exist.  
     */
    public function fetchID($column, $value)
    {
    ...
    public function create()
    {
        ...
        //$query = 'INSERT INTO `stickies` VALUES (1, \'hello\', 2, \'0000-00-00 00:00:00\',  \'0000-00-00 00:00:00\', \'yellow\')';
    ...
    public function exists(){
        ...
        //Just try to read, but don't update the current record.
        return $this->read(false);
    }

    /**
     * If update is set to true, then the read will result in updated values. 
     * @param type $update
     * @return type
     */
    public function read($update = true)
    {
        if($this->_key === null)
            return false;
        		
        $query = 'SELECT * FROM '.$this->_table.' WHERE BINARY '.$this->_col."='".DB::mysqli()->real_escape_string($this->_key)."'";
    ...
    /**
     * Returns the current active record's key in a string format.
     * @return string String representation of the current record's key.
     */
